...
class PurchaseOrdersListScreen extends StatefulWidget {
  ...
  const PurchaseOrdersListScreen({Key? key, this.initialStatus}) : super(key: key);
...
class _PurchaseOrdersListScreenState extends State<PurchaseOrdersListScreen> {
  ...
  Future<void> _loadPurchaseOrders() async {
    ...
    var pos = await _dbHelper.query('purchase_orders');
    pos.sort((a, b) => (b['orderDate'] as int).compareTo(a['orderDate'] as int));
    setState(() {
      _purchaseOrders = pos;
      _applyFilters();
      _isLoading = false;
  ...
  Future<void> _deletePurchaseOrder(String id, String supplierName) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete order from $supplierName?'),
        actions: [
        ...
        child: RefreshIndicator(
          ...
          child: _isLoading
              ...
              : _uiPurchaseOrders.isEmpty
                  ? const Center(
                      child: Text('No purchase orders found.', style: TextStyle(color: AppColors.white)),
                    )
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        DateTime orderDate = DateTime.fromMillisecondsSinceEpoch(po['orderDate']);
                        ...
                        return ListTile(
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => PurchaseOrderDetailsScreen(poId: po['id']),
                              ),
                            );
                          },
                          ...
                          trailing: Row(
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () => _deletePurchaseOrder(po['id'], po['supplierName'] ?? ''),
                              ),
                            ],
                          ),
                        );
